from core.data_provider import mnist, kth_action
...
def data_provider(dataset_name, train_data_paths, valid_data_paths, batch_size,
                  img_width, seq_length, seq_length_test, is_training=True):
    ...
    if dataset_name == 'action':
        ...
        if is_training:
            train_input_param = {'paths': train_data_list,
                       ...
                       'input_data_type': 'float32',
                       'name': dataset_name + ' iterator'}
            train_input_handle = datasets_map[dataset_name].DataProcess(train_input_param).get_train_input_handle()
            train_input_handle.begin(do_shuffle=True)
            test_input_handle = datasets_map[dataset_name].DataProcess(test_input_param).get_test_input_handle()
            test_input_handle.begin(do_shuffle=False)
            return train_input_handle, test_input_handle
        else:
            test_input_handle = datasets_map[dataset_name].DataProcess(test_input_param).get_test_input_handle()
